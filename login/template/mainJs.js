...
let valor = contenedor_img.css("width", img * 1000 + "%"); //calculo que cada imagen tenga un ancho de 100% del div en css
...
contenedor_img.css({ left: "-" + 1000 + "%" }); //pasa a primer lugar con la propiedad left, coge el 100% la imagen

function siguiente() {
  //ANIMACION EXTRA //Al pulsar boton
  contenedor_img.animate({ left: "-" + 500 + "%" }, 900, function() {
...
function anterior() {
  contenedor_img.animate({ left: 0 }, 900, function() {
    ...
    contenedor_img.css({ left: "-" + 1000 + "%" }); //indica el % que la pasa
  });
}
